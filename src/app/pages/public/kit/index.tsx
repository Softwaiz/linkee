...
import { Collection, CollectionSettings, db, User } from "@db/index";
...
export default async function PublicCollectionPage({ params, ctx, request }: RequestInfo) {
    ...
    let board: PubliclyVisibleCollection | null = await env.CONTENT_CACHE.get(`kit-${id}`, "json");
    if (!board) {
        board = await db
            .selectFrom("boards")
            .selectAll()
            ...
            .executeTakeFirst() as unknown as PubliclyVisibleCollection;

        await env.CONTENT_CACHE.put(`kit-${id}`, JSON.stringify(board), { expirationTtl: 60 * 3 });
    }
